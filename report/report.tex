...
\begin{enumerate}
  \item \textbf{Data ingestion} (\texttt{01}–\texttt{02}): FPB and FiQA are
  ...
  \item \textbf{Explainability} (\texttt{10}, \texttt{16}): SHAP regime-shift
    analysis on the fine-tuned model.
  \item \textbf{Econometrics} (\texttt{11}–\texttt{13}): Event study
...
configured pipeline made it straightforward to trace every design decision.
...
\begin{itemize}
  \item \path{python scripts/01_fetch_benchmarks.py}
  \item \path{python scripts/03_build_lm_lists.py}
  \item \path{python scripts/04_lexicon_baselines.py}
  \item \path{python scripts/05_lr_shap_benchmarks.py}
  \item \path{python scripts/06_finbert_benchmarks.py}
  \item \path{python scripts/08_eval_on_manual_annotations.py}
  \item \path{python scripts/08b_finetune_and_eval.py}
\end{itemize}
